...
object CustomerService {
  def main(args: Array[String]): Unit = {
    ...
    case class Hotelname(
        ...
        var length_of_last_stay: String,
    ...
    for (c <- customer_arr) {
      ...
      if (!result.isEmpty()) {
        ...
        cust.last_update = java.util.Calendar.getInstance().getTime().toString()
        ...
        reservation_status match {
          ...
          case "CHECKED IN" =>
            ...
            var hotel_details_upcoming_map = cust.hotelname.upcoming_stays.hotel_details
            var details_list = hotel_details_upcoming_map.get(hotel_name).get.to[ListBuffer]
            cust.hotelname.number_of_past_bookings +=1
            cust.hotelname.length_of_last_stay = details.length_of_stay
          ...
          case "CHECKED OUT" =>
            ...
            var details_list = hotel_details_upcoming_map.get(hotel_name).get.to[ListBuffer]
            cust.hotelname.number_of_past_bookings +=1
            cust.hotelname.last_purchase_date=booking_date
            cust.hotelname.length_of_last_stay = details.length_of_stay
            ...
            for (old_details <- details_list) {
              if (old_details.booking_date.equals(booking_date)) {
                ...
                {
                 hotel_details_upcoming_map = hotel_details_upcoming_map - hotel_name
                }else {
                	hotel_details_upcoming_map += (hotel_name -> details_list.toList)
